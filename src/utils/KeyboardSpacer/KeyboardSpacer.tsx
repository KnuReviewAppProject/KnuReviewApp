...
export const KeyboardSpacerContext =
  React.createContext<KeyboardSpacerContextType | null>(null);
...
export const useKeyboardSpacer = (): KeyboardSpacerContextType => {
...
export const KeyboardSpacerProvider: React.FC<KeyboardSpacerProviderProps> = ({
  children,
  noSpaceHeight,
}) => {
  const [height, setHeight] = useState<number>(noSpaceHeight ?? 0);
  const [keyboardFullHeight, setKeyboardFullHeight] = useState<number>(0);
...
export const KeyboardSpacer: React.FC<KeyboardSpacerProps> = ({
  ...
  const {height, setKeyboardFullHeight, keyboardFullHeight, KeyboardHeight} =
    useKeyboardSpacer();
  ...
  useEffect(() => {
    const updateKeyboardSpace = (e: KeyboardEvent) => {
      ...
      if (keyboardFullHeight !== e.endCoordinates.height) {
        setKeyboardFullHeight(e.endCoordinates.height);
      }
    ...
    const {updateListener, resetListener}: Record<string, KeyboardEventName> =
      Platform.select({
        android: {
